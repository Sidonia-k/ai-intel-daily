...
from ai_intel_daily.committee.cyber_committee import build_cyber_committee_analysis
...
def render_ai_stock_report(snapshot: dict[str, Any], report_date: str) -> str:
    ...
    cyber_committee_analysis = build_cyber_committee_analysis(snapshot)
    lines = [
        "# AI 相关股市信息研究报告",
        "",
        f"日期：{report_date}",
        ...
        "## 赛博投资委员会辅助分析",
        "",
        _render_cyber_committee(cyber_committee_analysis),
        "",
        "## AI 产业链分类观察",
        "",
...
def _render_cyber_committee(analysis: dict[str, Any]) -> str:
    lines = [
        "### 金融安全免责声明",
        "",
        str(analysis.get("financial_safety_disclaimer", "")).strip()
        or "- 暂无有效数据。",
        "",
        "### 事实基础",
        "",
        _render_list(analysis.get("facts_basis", [])),
        "",
        "### 角色视角",
        "",
    ]

    for role in analysis.get("role_views", []):
        if not isinstance(role, dict):
            continue
        name = str(role.get("name", "")).strip()
        focus = str(role.get("focus", "")).strip()
        if not name:
            continue
        lines.append(f"#### {name}")
        lines.append("")
        if focus:
            lines.append(f"- 关注重点：{focus}")
        lines.append("- 关键问题：")
        lines.append(_render_list(role.get("key_questions", [])))
        lines.append("- 本地规则观察：")
        lines.append(_render_list(role.get("local_observations", [])))
        lines.append("")
    ...
    lines.extend(
        [
            "### 委员会共识",
            "",
            _render_list(analysis.get("committee_consensus", [])),
            "",
            "### 主要分歧",
            "",
            _render_list(analysis.get("major_disagreements", [])),
            "",
            "### 不确定性",
            "",
            _render_list(analysis.get("uncertainties", [])),
            "",
            "### 后续观察指标",
            "",
            _render_list(analysis.get("follow_up_indicators", [])),
        ]
    )
    return "\n".join(lines).rstrip()
